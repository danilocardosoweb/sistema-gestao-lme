...
function calculatePeriodAverages(data, currentDate) {
    ...
    return {
        weekly: {
            dolar: formatNumber(calculateAverage(weeklyData.map(item => item.dolar))),
        ...
        monthly: {
            dolar: formatNumber(calculateAverage(monthlyData.map(item => item.dolar))),
