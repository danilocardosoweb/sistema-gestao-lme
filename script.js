...
async function updatePricesTable(prices) {
    ...
    for (const week of sortedWeeks) {
        ...
        for (const price of weekData) {
            ...
            const ptaxDisplay = ptaxRate ? formatDollarNumber(ptaxRate) : '-';
            ...
            row.innerHTML = `
                ...
                <td>${price.dolar !== '-' ? formatDollarNumber(price.dolar) : '-'}</td>
                <td>${ptaxDisplay}</td>
            `;
        ...
        if (validPrices.length > 0) {
            ...
            const ptaxAverage = validPtaxValues.length > 0 
                ? formatDollarNumber(validPtaxValues.reduce((a, b) => a + b, 0) / validPtaxValues.length)
                : '-';
            ...
            averageRow.innerHTML = `
                ...
                <td>${formatDollarNumber(weekAverages.dolar)}</td>
                <td>${ptaxAverage}</td>
            `;
